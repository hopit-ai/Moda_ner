...
**Rebuild the splits first.** The builder regenerates them from their public sources:

```bash
pip install datasets
python -m suite.text.build_splits --out-dir suite/text/data --download-fp
```

Sources are `arturayupov/womens-fashion-catalog` (MIT, published by Arcada LLC) and
Fashionpedia-derived synthetic text (CC BY 4.0). Labels are rule-derived, with no LLM
